...
export default function ChapterPage({ chapter }: ChapterPageProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/references/wiki/${chapter.file}`)
      ...
      .catch(() => {
        setError(`Chapter ${chapter.id} content is being generated. Please refresh shortly.`);
        setLoading(false);
      });
  }, [chapter.file]);
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {loading && (
        <div className="flex items-center justify-center py-20">
          ...
          <span className="ml-3 text-gray-400">Loading chapter content...</span>
        </div>
      )}

      {error && (
        <div className="bg-yellow-900/30 border border-yellow-700/50 rounded-lg p-6 text-yellow-200">
          <p className="font-semibold mb-2">⏳ Content Being Generated</p>
          <p>{error}</p>
        </div>
      )}
      ...
      {!loading && !error && content && (
        <div className="mt-12 pt-6 border-t border-gray-800 text-gray-500 text-sm">
          <p>📖 Content sourced from course materials and standard references.</p>
